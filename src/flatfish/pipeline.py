"""Pipeline orchestration for Flatfish processing."""
...
from flatfish.dataset import load_hf_dataset, iter_document_images, save_image
from flatfish.htr.engine import HTREngine
from flatfish.entities.extractor import EntityExtractor, consolidate_entities, load_entities
...
def run_pipeline(
    ...
    with Progress(
        ...
        dataset = load_hf_dataset(config, env)
        ...
        if not skip_summary:
            run_summary(config, env, progress=progress)
    ...
    if not skip_summary:
        console.print(f"  Summary: {config.output.summaries_dir}/")
    if not skip_build:
...
def run_extraction(
    ...
    for doc in iter_document_images(dataset, config, limit=limit):
